...
v_server_difficulty=$(or ${SERVER_DIFFICULTY},${server_difficulty})
v_server_challenge_len=$(or ${SERVER_CHALLENGE_LEN},${server_challenge_len})
v_server_challenge_buffer_size=$(or ${SERVER_CHALLENGE_BUFFER_SIZE},${server_challenge_buffer_size})
...
v_client_log_level=$(or ${CLIENT_LOG_LEVEL},${client_log_level})
...
v_server_env=SERVER_LOG_LEVEL="${v_server_log_level}" \
	...
	SERVER_DIFFICULTY="${v_server_difficulty}" \
	SERVER_CHALLENGE_LEN="${v_server_challenge_len}" \
	SERVER_CHALLENGE_BUFFER_SIZE="${v_server_challenge_buffer_size}" \
...
v_client_env=CLIENT_LOG_LEVEL="${v_client_log_level}" \
	CLIENT_SERVER_ADDR="${v_client_server_addr}" \
...
docker-server-run: docker-server-rm
	docker run \
		...
		-e "SERVER_DIFFICULTY=${v_server_difficulty}" \
		-e "SERVER_CHALLENGE_LEN=${v_server_challenge_len}" \
		-e "SERVER_CHALLENGE_BUFFER_SIZE=${v_server_challenge_buffer_size}" \
...
docker-client-run: docker-client-rm
	docker run \
		-e "CLIENT_LOG_LEVEL=${v_client_log_level}" \
