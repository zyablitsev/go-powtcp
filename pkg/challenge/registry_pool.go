...
// RegistryPool is the thread-safe map-based pool with TTL
// record invalidation support.
// RegistryPool uses double linked list to maintain FIFO order
// of inserted values.
...
// NewRegistryPool creates RegistryPool instance
...
func (c *RegistryPool) Set(key string) time.Time {
	c.mux.Lock()
	defer c.mux.Unlock()
...
func (c *RegistryPool) Get(key string) bool {
	c.mux.Lock()
	defer c.mux.Unlock()
...
func (c *RegistryPool) cleanup() error {
	c.mux.Lock()
	defer c.mux.Unlock()
